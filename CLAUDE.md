...
_Last updated: 2026-06-08 (Session 18 — 🔧 **記憶 + 藍圖修復**（founder 叫深查最新 playthrough「骰子覺醒」117 回合）· 揾到並修好 4 個問題：(1) **記憶死亡螺旋**（digest 卡 73 回合「倒帶」· root = Haiku 無視長度 → 螺旋；fix = Sonnet + 4096 + 脫困重壓縮 + 字數預算 + billing 同步 · PR #81 上 prod · 修復腳本重建 4 個現有檔）(2) **擲骰故事冇能力值**（0 擲骰/117 回合 · root = 面板生成唔知機制；fix = game_mode 傳入強制能力屬性 · PR #82）(3) **故事弧卡第 1 幕**（root = 硬寫死條件引用唔存在欄位；fix = extractor 定性判幕 · 刪 arc-dsl.ts · PR #82）(4) NPC 內心戲未啟用 = 設定問題（檔冇開）· director_verdict = 無害死碼。新增 hard rule #39（死亡螺旋）+ #40（blueprint coherence）。兩個 PR squash 上 main 已部署 prod。Open：B1「自動模式 AI 自選擲骰」未覆蓋（罕有）· 成人 digest 仍靠 Grok 可能長故事爆 · NPC 預設自動開（C tier 未做）· bible 仍生成 vestigial transition_condition。)_
