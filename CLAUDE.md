...
- **Model**：**非成人預設 → Claude 直連**（Standard = Sonnet · Pro = Opus · 真逐字串流 + prompt cache）；**成人 → Grok**（CrazyRouter · 不變 · hard rule #5）。⚠️ CrazyRouter buffer 晒所有 model，只有 Claude 直連先真串流。🆕 **2026-06-17（Session 20）**：Gemini 3.5 Flash / GPT-5.4 / DeepSeek V3.2 **加返做 picker opt-in 選項**（CrazyRouter · buffered · 前端打字咁出）—— Claude 維持每個 tier 嘅**預設**兼真串流。founder 從來**冇講過**要退役佢哋（light-core pivot 拎走佢哋只係「非成人要串流」嘅連帶後果 · 「用戶可揀 model」係核心原則）。**fal.ai LLM 端點查實唔啱做敘事**：淨係收單 prompt（冇 messages array → 塞唔到多回合 context）· 型號唔啱（gemini-2.5-flash / gpt-4.1）· 底層駁 OpenRouter。fal 維持淨係生圖用。詳見 ChatControls `PICKER_MODEL_IDS`。
...
39. **Cumulative-summary death-spiral · 一卡死就永世翻唔到身** — Session 18 (2026-06-08) 真實 prod bug：running digest（「故事前情」滾動摘要）一旦長過 output cap 就**永久卡死**。機制：prompt 叫「逐字保留 + 加新」→ digest 只長唔短 → 撞 maxOutputTokens → 截斷 → 舊 code 唔 advance through 就放棄 → **下次折疊更多回合 → 更長 → 又截斷**，單調惡化。骰子覺醒卡咗 44 回合，narrator 退返舊快照「倒帶」重講舊場景。**三個治本要素，缺一不可**：(a) prompt 俾**硬性字數預算** + 故事越長越要壓舊段；(b) **脫困機制** — 截斷時 retry 一次「首要任務=寫短」模式，打破螺旋；(c) **揀啱 model** — 實測 **Haiku 無視長度指令、唔肯壓縮已大嘅 digest**（1322→2273 字一次折疊就爆），**Sonnet 守指令**（2273→1714）。任何「累積式摘要 / compact」都要 bound length + 有 escape + 揀肯壓縮嘅 model。⚠️ 換 model 即同步 billing rate（computeTurnCredits 用 digestModelForRating · hard rule #4），否則收少。
40. **Blueprint coherence · 獨立並行生成兜唔埋** — Session 18：擲骰故事冇可擲屬性 + 故事弧全平台卡第 1 幕，**同一個根**。`generateStory` 將 mechanic / state panel / story arc / characters 喺**4 個獨立 parallel call** 生成，互相睇唔到 → 任何「A 引用 B」嘅 cross-reference 必然兜唔埋：(a) 面板生成器只收 locale、唔知用戶揀咗擲骰 → 出計數器唔出能力值 → 0 擲骰；(b) bible 嘅 arc `transition_condition` 引用 state schema 冇嘅欄位 → 永遠 false → 卡第 1 幕。鎖定設計（`pm/architecture/05`）本來就係「**一次過生成藍圖**」—— 並行拆開係 drift。**修法兩條**：要麼將共同決定（game_mode）**傳入**下游 call（B1：dice → 強制能力值），要麼**運行時用 AI 定性判斷**取代硬寫死 cross-reference（B2：arc 由 extractor 睇敘事判幕 · 原則 1 emergent）。教訓：**任何要兩個獨立 LLM 生成各自同意嘅硬 cross-reference 都係 bug class**（同 #28 LLM-output enum drift 同源）。
...
_Last updated: 2026-06-17 (Session 20 — 🖼️ **Wave 3 + UI/UX + 修復一批**（全部 squash 上 main + 部署 prod + 瀏覽器實測）：(1) **Wave 3 角色設計圖**（fal.ai gpt-image-2 · 全程一致角色樣 · PR #94 · migration 0069 已 apply）(2) **UI/UX 6 批大改**（互動回饋 / loading / 退役文案清理含法律頁 / 視覺打磨 / 死碼清 / design token 統一 · PR #95）(3) **跨網域連結 CORS 修復**（subdomain 拆分以嚟一路有嘅手尾：產品網 footer/設定/play/生圖視窗嘅行銷 link 會背景 RSC-預載 kieio.com → 跨域 CORS console 紅字。新 `CrossSubdomainLink`：跨網域→普通 `<a>` 唔預載 · 同域→`<Link>` · 全站 6 處 · PR #96 · 實測 0 跨域預載）(4) **建故事最少 20 字提示**（掣灰住有解釋 · founder 自己撞到 · PR #96）(5) **加返 model 選項**：Gemini 3.5 Flash / GPT-5.4 / DeepSeek V3.2 重返 picker 做 opt-in（Claude 維持每 tier 串流預設 · founder 從無講過要退役 · **fal LLM 端點查實唔啱** = 單 prompt / 型號唔啱 / 駁 OpenRouter · PR #97 · 實測 5 隻齊）。**Session 19（前段同期）** = 記憶手術 M2/M3/M4（PGroonga 中文關鍵字檢索 + 信念時間軸復活 + 同名合併判官 · migration 0064-0068）+ 夜間自動試玩 harness（qa-nightly · GitHub Actions）· M1 中文 embedding 擱低（CrazyRouter 中文向量單路由/實測更差）· 全部 ship。)_

_Prev: 2026-06-10 (Session 18 續 — 🧹 **審計 batch 上 prod**（PR #83 · fae3bc6 · 淨 -840 行）：產品審計（3 agent：安全/成本/正確性）+ 技術架構審計（3 agent：drift+死碼地圖/結構/擴展性）→ founder 揀「A+B 全做」一次過修：(1) **C1** NPC 內心戲付費 tier 自動開（兩條創建路徑 · guarded）(2) **計費**：建故事多收 3 倍修正（跑 Haiku 收 Haiku）· 免費回合堵塞（穿底 clamp-charge 扣清餘額）· SFW 生圖按真實 Grok 成本重訂（插圖 100/壁紙 120 · 舊價 ~0% 毛利）· NPC 收費文案改「按實際用量」（UI≠ledger 信任）· 過濾 Haiku call 決定吸收入 2× markup（寫低喺 BACKGROUND_RESERVE_TOKENS）(3) **安全**：playthroughs 系統欄位（running_summary/current_state/mention_roster）user-client 可寫 → turn route 改 service-role 寫 + **migration 0062** trigger revert 非 service-role 寫入（堵 hr#5 繞過：塞料入自己 SFW 檔氹 prompt 送 Anthropic）(4) **C2 死碼大清理**（-1455 行）：成條 inert Director/GM 鏈（hints/refine · NPC-L2 apply+persist · 靈魂寫入 after()）+ **刪 experience-writer.ts + sediment.ts**（表/RPC 留 Wave 2）+ narrator 工具時代機器 + refusal canned fallback + NPC flat-6 · **計費驗證前後一致**（extractor 計費桶未掂）(5) **加固**：MODELS⊆MODEL_PRICING 開機斷言 · game_system Zod (6) **去重漂移對**：pickDigestModel→models.ts 單一來源 · BACKGROUND_RESERVE_TOKENS 共用 gate+charge · SUBSCRIPTION_TIER_ORDER 取代 6 處手抄 (7) **DB perf**：**migration 0063** = 40 條 RLS `(select auth.uid())` initPlan 重寫 + 刪 3 個從未用 HNSW 索引（EXPLAIN 證實）+ 5 個 FK 索引；running_summary+game_system 摺入主 select（每回合 -2 round-trip）。0062+0063 已按次序 apply（code 先 → 鎖後）+ sanity 全過。**剩 backlog（C 組 · 唔 blocking）**：vitest + 5 個高價值測試目標 · turn route onFinish 抽 helper · 向量 age-out（儲存炸彈）· 背景 LLM 搬離 turn lambda · 生圖孤兒清理 · 自動模式 AI 自選擲骰 gap · 擲骰排除資源條 · transition_condition/red_lines 改 optional（殘留生成 · 無害）· 成人 digest 長故事 Grok 長度風險。)_

_Prev: 2026-06-08 (Session 18 — 🔧 **記憶 + 藍圖修復**（founder 叫深查最新 playthrough「骰子覺醒」117 回合）· 揾到並修好 4 個問題：(1) **記憶死亡螺旋**（digest 卡 73 回合「倒帶」· root = Haiku 無視長度 → 螺旋；fix = Sonnet + 4096 + 脫困重壓縮 + 字數預算 + billing 同步 · PR #81 上 prod · 修復腳本重建 4 個現有檔）(2) **擲骰故事冇能力值**（0 擲骰/117 回合 · root = 面板生成唔知機制；fix = game_mode 傳入強制能力屬性 · PR #82）(3) **故事弧卡第 1 幕**（root = 硬寫死條件引用唔存在欄位；fix = extractor 定性判幕 · 刪 arc-dsl.ts · PR #82）(4) NPC 內心戲未啟用 = 設定問題（檔冇開）· director_verdict = 無害死碼。新增 hard rule #39（死亡螺旋）+ #40（blueprint coherence）。兩個 PR squash 上 main 已部署 prod。)_

_Prev: 2026-06-02 (Session 17 — 🆕 **LIGHT-CORE PIVOT**（見頂部 banner）· founder 親手玩完發現比 raw LLM 更慢+更唔一致+冇差異 → 拎走 GM/四層仲裁、非成人轉 Claude 直連（Standard=Sonnet / Pro=Opus · 真串流 + cache）、成人維持 Grok、記憶降做衞生、擲骰移出核心 · 取代 Session 16 四層架構（`pm/architecture/decisions.md` ADR-006 supersedes ADR-001）· 市場研究 + 7 人策略委員會驗證 · Wave 1 ✅ 實作完 + build + 兩輪深度審計修好 3 個 ship-blocker（真串流原本死咗 / 非成人 CSAM 漏 / cached token 雙收費 — hard rule #4）· branch `feat/light-core-wave1` 3 commit · **未 merge 未部署** · W3 migration `0057` 待 confirm · Wave 2 = 角色 MD+keyword（MemPalace 式）· 同場清走 stale docs（18 audit reports + 舊 scratch）+ 更新 CLAUDE/STATUS/architecture · 🏁 next = 部署去 preview 俾 founder 試真串流/Sonnet 質素)_
